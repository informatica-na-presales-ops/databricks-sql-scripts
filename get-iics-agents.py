...
def main_job(repeat_interval_hours: int = None):
    ...
    updated_at_start = pg.data_lake_postgres.get_iics_agents_max_record_updated_at(
        pg_cnx
    )
    ...
    adv_configs = list(
        dbx.cnx.get_iics_advanced_cluster_configs(dbx_cnx, close_cnx=False)
    )
    pg.data_lake_postgres.batch_upsert_iics_advanced_cluster_configs(
        pg_cnx, adv_configs
    )

    adv_instances = list(
        dbx.cnx.get_iics_advanced_cluster_instances(dbx_cnx, close_cnx=False)
    )
    pg.data_lake_postgres.batch_upsert_iics_advanced_cluster_instances(
        pg_cnx, adv_instances
    )
