...
def get_iics_agents(
    cnx: databricks.sql.client.Connection, record_updated_at_start, close_cnx: bool
):
    log.info(
        f"Getting IICS agents in AWS-TS region updated on or after {record_updated_at_start}"
    )
    sql = """
        select * from (
            select
...
def get_iics_user_roles(cnx: databricks.sql.client.Connection, updated_on_start):
    log.info(
        f"Getting IICS user roles in AWS-TS region updated on or after {updated_on_start}"
    )
    sql = """
        select
